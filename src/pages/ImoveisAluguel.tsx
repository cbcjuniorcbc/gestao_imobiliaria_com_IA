...
const ImoveisAluguel = () => {
  ...
  const loadData = async () => {
    if (window.electronAPI && 'getImoveis' in window.electronAPI) {
      ...
      if (result.success) {
        setImoveis(result.data.filter((i: Imovel) => i.tipo_negocio === 'Aluguel'));
      }
    } else {
      setImoveis(mockImoveis.filter(i => i.tipo_negocio === 'Aluguel'));
    }
    setLoading(false);
  };
      ...
      <header className="bg-card border-b shadow-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">Imóveis para Aluguel</h1>
              <p className="text-sm text-muted-foreground">Todos os imóveis disponíveis para locação</p>
            </div>
          </div>
